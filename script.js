...
summonBtn.addEventListener("click", async () => {
    ...
    // We do this separately to ensure the browser sees the user gesture
    if (!device || !characteristic) {
        try {
            await connectBluetooth();
            // Optional: short delay to let notifications start
            await new Promise(r => setTimeout(r, 500)); 
        } catch (err) {
            console.error("Connection failed:", err);
            return; // Stop if they cancel the pairing dialog
        }
    }

    // 2. NOW CHECK STATE AND RUN LOGIC
    if (state === "idle") {
        summonBot();
...
async function sendBleSignal(signal, isPriority = false, zoneID) {
    ...
    try {
        ...
        console.log(`BLE sent. Signal: ${signal} | Coordinates: ${stableCoords.x.toFixed(2)}m, ${stableCoords.y.toFixed(2)}m | ZoneID: ${zoneID}`);
    } catch (error) {
...
function checkAndTrilaterate() {
    if (latestDistances["0"] !== null && latestDistances["1"] !== null && latestDistances["2"] !== null) {
        ...
        if (coords) {
            stableCoords.x = coords.x;
            stableCoords.y = coords.y;
            console.log("📍 Position Calculated:", stableCoords);
        }
    }
}
